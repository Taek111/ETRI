...
def arrange(file):
    ...
    for f in range(len(body_info)):
        new_body_frame = list()
        for a in range(n_skeletons): #available body information
            new_body_frame.append(body_info[f][available_body_id[a]])
        for e in range(n_skeletons, 6): #empty body information
            new_body_frame.append(empty_body)
        new_body_info.append(new_body_frame)

    return new_body_info

def arrange_body_info(body_info, mode):

    if mode == "C001" or mode == "C002":
        n_max_skeletons = 1
    elif mode == "C003":
        n_max_skeletons = 2
    else:
